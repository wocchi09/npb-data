(function(){
  ...
  function renderTwoStrike(){
    ...
    h+=`<section class="card" style="border-top:4px solid ${teamColor(p.team)}"><div class="team-title"><span class="team-dot" style="--team:${teamColor(p.team)}"></span><h3>${esc(p.name)} <small>${esc(p.team)} / ${esc(p.hand||"-")}</small></h3></div><div class="kpis"><div class="kpi"><b>${p.pitches}</b><span>2ストライク後の投球</span></div><div class="kpi"><b>${pct(p.k_finish_rate)}</b><span>三振決着球率</span></div><div class="kpi"><b>${pct(p.whiff_rate)}</b><span>空振り率</span></div><div class="kpi"><b>${esc(fin.pitch_type||"-")}</b><span>高決着率の球種</span></div></div><div class="note">${esc(state.data.notes?.two_strike||"")}</div></section><div class="split-grid"><section class="card"><h3>球種配分</h3><table class="pitch-table"><thead><tr><th>球種</th><th>割合</th><th>三振決着</th><th>空振り</th><th>平均球速</th></tr></thead><tbody>${(p.pitch_types||[]).map(x=>`<tr><td><b>${esc(x.pitch_type)}</b><br><small>${esc(x.top_zone||"")}</small></td><td>${pct(x.share)}</td><td>${pct(x.k_finish_rate)}</td><td>${pct(x.whiff_rate)}</td><td>${x.avg_speed==null?"-":num(x.avg_speed,1)+"km/h"}</td></tr>`).join("")}</tbody></table></section><section class="card"><h3>左右打者への使い分け</h3>${splitBlock("右打者",p.vs_right)}${splitBlock("左打者",p.vs_left)}<h3 style="margin-top:20px">球種シェア</h3>${(p.pitch_types||[]).slice(0,5).map(x=>`<div class="bar-row"><b>${esc(x.pitch_type)}</b><div class="track"><div class="fill" style="width:${Math.max(2,(x.share||0)*100)}%"></div></div><span>${pct(x.share)}</span></div>`).join("")}</section></div>`;
